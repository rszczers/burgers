...
Każde odwzorowanie liniowe \(\mathrm{X}:\mathrm{C}_p^\infty(M)\to\R\) spełniające warunek:
\begin{equation}\label{eq:differentiation}
    \mathrm{X}(fg) = (\mathrm{X}f)g(p) + f(\mathrm{X}g)(p)
\end{equation}
nazywamy \emph{różniczkowaniem} w punkcie \(p\). Na zbiorze wszystkich takich różniczkowań w punkcie \(p\) można zadać strukturę \(n\)-wymiarowej \(\R\)-przestrzeni liniowej z działaniami
\begin{align*}
    (X + Y)f := Xf + Yf, &\quad (rX)f := r(Xf),\ \text{gdzie}\ f\in\mathcal{F}(M),\ r\in\R.
\end{align*}
Taką strukturę nazywamy \textbf{przestrzenią styczną} w \(p\) i oznaczamy przez \(T_pM\), a każdy \(X\in T_pM\) nazywamy \emph{wektorem stycznym} w \(p\).

\subsection{Wiązki wektorowe}
...
\subsection{Tensory i formy różniczkowe}
...
\textbf{Iloczynem zewnętrznym \(p\)-wektorów} (odpowiednio: \textbf{\(p\)-form}) \(t_i\in\Exterior^{p_i}(V)\), \(s^i\in\Exterior^{p_i}(V^\ast)\), \(i=1,\dots, k\), nazywamy odpowiednio tensory
\begin{align*}
...
\subsection{Grupy dyfeomorfizmów}
...
\subsection{Rozmaitości Riemannowskie}
