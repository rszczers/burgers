...
\textbf{Formą objętości} na \(n\)-rozmaitości \(M\) nazywamy każdą nieznikający na \(M\) \(n\)-formę \(\mu\in\Omega^n(M)\) i mówimy, że \(M\) jest orientowalna, jeśli na \(M\) można określić formę objętości.
...
Jeśli \(F: (M, \mu_M)\to(N, \mu_N)\) jest dyfeomorfizmem między rozmaitościami zorientowanymi, to powiemy, że \(F\) \emph{zachowuje (odwraca) orientację}, jeśli \(F^\ast \mu_M = \mu_N\) (\(F^\ast \mu_M = -\mu_N\)).
